...
def get_tweets(trend,date):
    enddate = date+datetime.timedelta(days=2)
    ...
    bearer_token=""
    rule = gen_rule_payload(trend+" has:geo",from_date=date.isoformat() ,to_date=enddate.isoformat(), results_per_call=100) # testing with a sandbox account
    rs=ResultStream(rule_payload=rule,max_results=10000, username=username,endpoint=endpoint, password=password)
...
def refine_tweet(tweet,trend,date):
    ...
    if tweet["coordinates"]:
        new_tweet["location"]=str(tweet["coordinates"]["coordinates"][0]) +" "+ str(tweet["coordinates"]["coordinates"][1]); 
    else:
        new_tweet["location"]= "-1 -1";
